...
本项目目前尚未选定统一许可证。核对时，上游机器人项目未提供项目根目录许可证。
公开源码并不自动代表允许再分发或重新授权；在公开发布派生代码前，需要向上游作者确认授权和许可证。

仓库内第三方库继续遵循各自目录中的许可证文件，分发前请阅读 [NOTICE](NOTICE.md)。

本仓库的目标是公开分享源码、记录学习过程并促进交流。目前不将其表述为符合 OSI
许可证定义的开源发行版，因为上游代码的授权条款仍需进一步确认。
